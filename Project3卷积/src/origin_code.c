...
int main()
{
    ...
    kernel kernel;
    kernel = initialKernel(kernel,1,5,5);  //在此修改kernel的size/row/col，在intialKernel()中修改kernel初始值。
    coved_data data;
    start = clock();
    // cov的输入参数为(stride_x,stride_y,padding_x,padding_y,kernel,image)
    data = cov(1, 1, 0, 0, kernel, image);
    finish = clock();  //计算时间
    printf("%lfs\n", (float)(finish - start) / (float)1000000);
    WriteBMP(data, "output_image.bmp");  //传出灰度图的文件名
    free_image(&image);
    free_kernel(&kernel);
    return 0;
}
