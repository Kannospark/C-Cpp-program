...
ImgArr read_Img(const char* filename) // 读入图像
{
    ...
    for(i = 0; i < number_of_images; ++i)
    {
        ...
        imgarr->ImgPtr[i].ImgData=(float**)malloc(n_rows*sizeof(float*));  //先给imgdata双重指针申请一块空间，代表行变量
        for(r = 0; r < n_rows; ++r)
        {
            imgarr->ImgPtr[i].ImgData[r]=(float*)malloc(n_cols*sizeof(float));  //再给imgdata的指针每个申请一块空间，代表列向量
            for(c = 0; c < n_cols; ++c)
            {
